...
export function time_ago(value){
  ...
  const now     = DateTime.now()
  const diff_mins = now.diff(future.diff(now, 'minutes').toObject().minutes);
  const diff_hours = now.diff(future.diff(now, 'hours').toObject().hours);
  const diff_days = now.diff(future.diff(now, 'days').toObject().days);
